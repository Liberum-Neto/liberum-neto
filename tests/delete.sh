...
$CLI_BIN config-node $N3 add-bootstrap-node "${N1_ID}" $N1_ADDR
$CLI_BIN start-node $N2
$CLI_BIN start-node $N3
...
DOWNLOAD_3_SUCCESS=$($CLI_BIN download-file $N3 "${FILE_ID}")
should_contain "$DOWNLOAD_3_SUCCESS" "${FILE_CONTENT}"

# # delete file
DELETE_3_FAIL=$($CLI_BIN delete-object $N3 "${FILE_ID}")
...
DELETE_2_FAIL=$($CLI_BIN delete-object $N2 "${FILE_ID}")
...
DELETE_1_FAIL=$($CLI_BIN delete-object $N1 "${FILE_ID}")
...
DOWNLOAD_3_FAIL=$($CLI_BIN download-file $N3 "${FILE_ID}")
...
# cleanup
$CLI_BIN stop-node $N1
$CLI_BIN stop-node $N2
killall liberum_core &> /dev/null
rm "$FILE_NAME"
