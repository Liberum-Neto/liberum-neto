...
impl Node {
    ...
    pub async fn provide_file(&mut self, path: PathBuf) -> Result<String> {
        ...
        let object = proto::PlainFileObject {
    ...
    pub async fn download_file(&mut self, id: String) -> Result<Vec<u8>> {
        ...
        for peer in &providers {
            ...
            match file_receiver.await {
                ...
                Ok(Ok(file)) => {
                    let hash = bs58::encode(blake3::hash(&file.data).as_bytes()).into_string();
                    if hash != id_str {
                        debug!(
                            node = self.name,
                            from = format!("{peer}"),
    ...
    pub async fn publish_file(&mut self, path: PathBuf) -> Result<String> {
        let id_key: libp2p::kad::RecordKey =
            liberum_core::get_file_id(&path).await.inspect_err(|e| {
                error!(
                    err = e.to_string(),
                    path = format!("{path:?}"),
                    "Failed to hash file"
        ...
        let id_str = liberum_core::file_id_to_str(id_key.clone());
        ...
        let object: TypedObject = proto::PlainFileObject {
            name: path.file_name().unwrap().to_str().unwrap().to_string(),
            content: data,
        }
        .into();

        let obj_id: proto::Hash = blake3::hash(&object.data)
            .as_bytes()
            .to_vec()
            .as_slice()
            .try_into()
    ...
    #[message]
    pub async fn provide_object(&mut self, object: TypedObject) -> Result<String> {
        let id: proto::Hash = blake3::hash(&object.data)
            .as_bytes()
            .to_vec()
            .as_slice()
            .try_into()
    ...
    #[message]
    pub(crate) fn get_object_from_vault(&mut self, key: proto::Hash) -> Option<TypedObject> {
